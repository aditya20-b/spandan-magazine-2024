...
export default function AdminPage() {
    ...
    setMatchResults((prevResults) => {
      return prevResults.map((r) => {
        if (r.id === result.id) {
          const updatedResult = {
            ...r,
            [field]: ["team1", "team2", "day", "sport"].includes(field)
              ? value
              : parseInt(value) || 0,
          };
  ...
  const deleteMatch = (result: MatchResult) => {
    setMatchResults((prevResults) =>
      prevResults.filter((r) => r.id !== result.id)
    );
    setChangedMatchResults((prevChanged) => [
      ...prevChanged,
      { ...result, _delete: true },
  ...
  const deleteStanding = (standing: Standing) => {
    setStandings((prevStandings) =>
      prevStandings.filter((s) => s.id !== standing.id)
    );
    setChangedStandings((prevChanged) => [
      ...prevChanged,
      { ...standing, _delete: true },
    ]);
  };
    ...
    <div className="container mx-auto p-4">
      ...
      <Tabs defaultValue="standings">
        ...
        <TabsContent value="matches">
          ...
          <Table>
            <TableCaption>
              Match Results for {selectedSport} on {selectedDay}
            </TableCaption>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {matchResults
                .filter(
                  (result) =>
                    result.day === selectedDay && result.sport === selectedSport
                )
                .map((result) => (
          ...
          <Button onClick={addNewMatch} className="mt-4">
            Add New Match
          </Button>
        </TabsContent>
      </Tabs>
